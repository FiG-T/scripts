...
core_GLM_scores_formatted_phased <- feather::read_feather(
  path = "~/Documents/data/lhf_d/feather/core_GLM_scores_formatted_phased_ds_08_2024.feather"
)

# importing the list of significant hits  
core_GLM_min_null_clustered <- feather::read_feather(
  path = "~/Documents/data/lhf_d/feather/core_GLM_scores_mins_ds_clustered_08_2024.feather"
)
...
core_GLM_min_null_clustered$Map.Locus <- factor( # arrange the locus into the order you want to plot them in
  x = core_GLM_min_null_clustered$Map.Locus, 
  levels = c(
...
#ggplot2::ggsave(
  filename = "/Users/finleythomas/Library/CloudStorage/OneDrive-UniversityCollegeLondon/Results/lhf/figures/lhf_hits_loci_overview_10_2024.png",
  plot = last_plot(),
  bg = "transparent", 
  width = 11, 
  height = 7
...
ggplot2::ggplot(
  data = core_GLM_min_null_clustered, # using the significant hits 
  mapping = aes(
...
ggplot2::ggsave(
  filename = "/Users/finleythomas/Library/CloudStorage/OneDrive-UniversityCollegeLondon/Results/lhf/figures/lhf_hits_loci_dotplot_10_2024.png",
  plot = last_plot(),
  bg = "transparent", 
  width = 11, 
  height = 7
...
```{r sort_lhf_meta_names}
# get names 
names_st2_meta <- names(lhf_gt_pc_st2_meta)

# create vector of position names
names_st2_meta_pos <- names_st2_meta[c(43:length(names_st2_meta))]
# remove the pos_ prefix
names_st2_meta_pos <- stringr::str_replace(
  string = names_st2_meta_pos, 
  pattern = "pos_", ""
)

# sort into order
names_st2_meta_pos <- sort(
  as.numeric(
    names_st2_meta_pos
  )
)
...
lhf_gt_pc_st2_meta <- lhf_gt_pc_st2_meta %>%
  select(
    names(lhf_gt_pc_st2_meta[c(1:42)]), 
    names_st2_meta_pos
  )
...
lhf_st2_GLM_scores <- core_GLM_multi_scorer_parallel(
  ...
  null_formula = "~ PC1 + PC2 + PC3 + PC4 + (1|iso2)", 
  format_table = TRUE, 
  output_file_stem = "/Users/finleythomas/Library/CloudStorage/OneDrive-UniversityCollegeLondon/Results/lhf/lhf_GLMs_chisq_st2_scores_WorldClim.csv"
)
...
lhf_st2_GLM_scores_paleo <- core_GLM_multi_scorer_parallel(
  ...
  null_formula = "~ PC1 + PC2 + PC3 + PC4 + (1|iso2)", 
  format_table = TRUE,
  output_file_stem = "/Users/finleythomas/Library/CloudStorage/OneDrive-UniversityCollegeLondon/Results/lhf/lhf_GLMs_chisq_st2_scores_PastClim.csv"
)
...
st2_GLM_scores_worldclim <- read.csv(
  file = "/Users/finleythomas/Library/CloudStorage/OneDrive-UniversityCollegeLondon/Results/lhf/lhf_GLMs_chisq_st2_scores_WorldClim.csv"
)

st2_GLM_scores_pastclim <- read.csv(
  file = "/Users/finleythomas/Library/CloudStorage/OneDrive-UniversityCollegeLondon/Results/lhf/lhf_GLMs_chisq_st2_scores_PastClim.csv"
)

st2_GLM_scores <- rbind(
    st2_GLM_scores_worldclim, 
    st2_GLM_scores_pastclim
  )
  

snps_run_st2 <- unique(sort(st2_GLM_scores$snp_pos))

snps_run_st2 <- str_c(
  "pos_", snps_run_st2
)

lhf_gt_pc_st2_meta_extra <- lhf_gt_pc_st2_meta %>%
  select(
    !matches(snps_run_st2)
  )

names(lhf_gt_pc_st2_meta_extra)

core_GLM_multi_scorer_parallel( # see interest SNPs 
    input = lhf_gt_pc_st2_meta_extra %>%
      filter( ! iso2 %in% non_paleo_iso), 
    start_col = 44, 
    num_cores = 1, 
    test_formula = c(test_formulas, test_formulas_paleo), 
    null_formula = "~ PC1 + PC2 + PC3 + PC4 + (1|iso2)", 
    output_file_stem = "/Users/finleythomas/Library/CloudStorage/OneDrive-UniversityCollegeLondon/Results/lhf/lhf_GLMs_chisq_st2_scores_Extras.csv",
    format_table = TRUE
)

st2_GLM_scores_extra <- read.csv(
  file = "/Users/finleythomas/Library/CloudStorage/OneDrive-UniversityCollegeLondon/Results/lhf/lhf_GLMs_chisq_st2_scores_Extras.csv"
)

unique(st2_GLM_scores_extra$snp_pos)

st2_GLM_scores <- rbind(
    st2_GLM_scores, 
    st2_GLM_scores_extra
  )

feather::write_feather(
  x = st2_GLM_scores,
  path = "/Users/finleythomas/Library/CloudStorage/OneDrive-UniversityCollegeLondon/Results/lhf/lhf_GLMs_chisq_st2_scores_all.feather"
)
```
...
lhf_st2_GLM_scores <- read.csv(
  file = "/Users/finleythomas/Library/CloudStorage/OneDrive-UniversityCollegeLondon/Results/lhf/lhf_GLMs_chisq_st2_scores.csv"
)
unique(sort(lhf_st2_GLM_scores$snp_pos))
...
lhf_st2_GLMM_scores <- st2_GLM_scores %>%
  filter(!is.na(Chisq))
...
# create a table with the most significant covariates for each significant SNP
...
unique(lhf_st2_GLMM_min$rCRS_pos)

feather::write_feather(
  x = lhf_st2_GLMM_min, 
  #path = "~/Documents/data/lhf_d/feather/lhf_st2_GLMM_min_LAT_08_2024.feather"
)
feather::write_feather(
  x = lhf_st2_GLMM_min, 
...
length(unique(lhf_st2_GLMM_min$rCRS_pos))
