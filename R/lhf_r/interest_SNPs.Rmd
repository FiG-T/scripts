...
interest_snps <- interest_snps %>%
  #filter(hit == 1) %>% # only include significant hits from GLM
  filter(total_lat_glm == 6 | total_precip_glm ==6) %>% # only include sites significant in all runs 
  filter(test == "all") %>%
  filter(pval < 0.05/(73*2)) %>%
  select(pos,total_lat_glm, total_precip_glm, pval, covariate, Map.Locus, Shorthand, classification, aln_start, aln_end) 

# create vector of lat SNPs
interest_snps_lat <- unique(
  c(
  interest_snps$pos[
    interest_snps$covariate == "lat" & interest_snps$total_lat_glm == 6 
  ]
 )
)
...
interest_snps_precip_yr <- unique(
  c(
  interest_snps$pos[
    interest_snps$covariate == "precip" & interest_snps$total_precip_glm == 6
  ]
 ) 
)
...
interest_snps_loci <- interest_snps %>%
  ...
  distinct() %>%
  mutate(
    per_snp = (N/(aln_end - aln_start)) * 100 # calculate the % mutations per loci 
  )
...
-   Latitude hits are more frequent in protein-coding regions

-   There are more hits for precipitation than latitude in the membrane
    attachment site

-   All hits withing the control region are also within the membrane attachment
    site.

-   There are only latitude related hits within the tRNA regions (bacteria
    paper?)
...
GLM_chi_multi_scorer <- function(
        ...
        start_col = 21, # where the genotype information starts
        col_to_use = "NULL",
  ...
  if (col_to_use == "NULL") {
    col_pos <- c(
      start_col:ncol(input)  # define the columns where the genotype data is held
    ) 
  } else {
    col_pos <- col_to_use 
  }
...
test_formulas <- c(
  "~ patr_KC345983 + tmp_max + (1|iso2)", 
  "~ patr_KC345983 + tmp_min + (1|iso2)",
  "~ patr_KC345983 + tmp_yr + (1|iso2)",
  "~ patr_KC345983 + tmp_range_drl + (1|iso2)",
  "~ patr_KC345983 + isotherm + (1|iso2)",
  "~ patr_KC345983 + tmp_seasonality + (1|iso2)",
  "~ patr_KC345983 + tmp_range_yr + (1|iso2)", 
...
lat_interest_chi_sq$covariate <- str_extract(
  ...
  pattern =  "tmp_max|tmp_min|tmp_yr|tmp_range_drl|isotherm|tmp_seasonality|tmp_range_yr|lat", 
...
lat_interest_chi_sq_sum <- lat_interest_chi_sq %>%
  filter(`Pr(>Chisq)` <= (0.05/(length(interest_snps_lat)*7))) %>% # pvalue adjusted for the number of groups
  group_by(covariate) %>%
  reframe( N = n())

lat_interest_chi_sq_sum

# create a table with the most significant covariates for each SNP
lat_interest_chi_sq_min <- lat_interest_chi_sq %>%
  filter(`Pr(>Chisq)` <= (0.05/(length(interest_snps_lat)*7))) %>%
  filter(covariate %in% c(
    "tmp_min","tmp_max", "tmp_yr", "tmp_range_yr", "tmp_range_drl", "tmp_seasonality", "isotherm"
    )) %>%
  group_by(snp_pos) %>%
  slice_min(`Pr(>Chisq)`) %>%
  group_by(covariate) %>%
...
# initiate ggplot 
ggplot(
  data = lat_interest_chi_sq_sum, # using the summary data created above 
  mapping = aes(
    x = covariate, 
    y = N,  # show the number of significant hits 
    fill = covariate  # colour by the environmental data used 
  )
) + 
  geom_col(
    stat = "identity", 
...
# create second plot showing the strongest hits for each SNP
ggplot(
...
# change annoying ISO2 codes (changed since the database was created)
...
# merge the plant metadata with the original covariates 
interest_gt_data_meta <- left_join(
  interest_gt_data[,c(3:19)], 
  lhf_wcvp_summary_tb, 
  by = c("iso2")
) %>%
  distinct()
...
# merge the metadata (inc. plants) with the disease summary
interest_gt_data_meta <- left_join(
...
interest_gt_data_meta <- interest_gt_data_meta %>%
  ...
  # convert the number of outbreaks to a rank value
  mutate(
...
# add suffix onto positions of interest
precip_snps_col_names <- paste0(
  "pos_", 
...
precip_interest_gt_data <- interest_gt_data %>%
  select(
    c(
      samples:patr_KC345983, # select metadata columns 
      precip_snps_col_names
      )
    )

# combine with disease and plant metadata 
precip_interest_gt_data <- left_join(
  x = precip_interest_gt_data, 
  y = interest_gt_data_meta[, c(5, 18:26,29:32)], 
  by = "iso3"
)
...
precip_interest_gt_data <- precip_interest_gt_data %>%
  select(
    c(
      samples:patr_KC345983, 
      native:outbreaks_area_rank, 
      pos_156:pos_16351
    ) 
  )

# Scale the values 
col_to_scale <- c(20:27, 30,31)
...
# define test formulas
test_formulas <- c(
  "~ patr_KC345983 + native_area + (1|iso2)", 
  "~ patr_KC345983 + endemic_area + (1|iso2)",
  "~ patr_KC345983 + outbreaks_area + (1|iso2)",
  "~ patr_KC345983 + outbreaks_area_rank + (1|iso2)",
  "~ patr_KC345983 + native_area_rank + (1|iso2)",
  "~ patr_KC345983 + endemic_area_rank + (1|iso2)",
...
# remove additional hits (see below)
precip_interest_chi_sq <- precip_interest_chi_sq %>%
  filter(snp_pos %in% interest_snps_precip_yr)

# NA values are present for the null models 
precip_interest_chi_sq$covariate[is.na(precip_interest_chi_sq$Chisq)] <- "null"

# check that the only SNPs present are the interest ones (should match chunk above)
sort(unique(precip_interest_chi_sq$snp_pos))
```

> Care has to be taken here to remove additional hits (selected by error) - this
> arises when "pos_125" also matches to "pos_1252" (for example).
...
ggplot(
  ...
  geom_segment( # add bars in with mtDNA loci positions
    data = mt_loci_pos,
    ggplot2::aes(
      x = aln_start, 
      xend = aln_end,
  ...
  geom_point( # add points for the pval for each covariate per SNP
    mapping = aes(
      colour = covariate,
      shape = covariate
    ), 
  ...
  ggplot2::scale_colour_manual( # add in colour values 
    values = c( 
      native_area = "orchid",
  ...
  scale_shape_manual(
    values = c(
      15,0,17,2,19,1,8,4,25,8 # define shapes
    )
  ) +
  ggplot2::geom_line(
    y = -1*log10(0.05/(79*4)), 
...
```{r precip_summary}
n_snp_hits_precip <- length(unique(interest_snps_precip_yr))
n_tests <- length(unique(test_formulas))
