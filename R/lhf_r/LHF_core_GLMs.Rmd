...
```{r generate_hits_subset}

core_GLM_hit_snps <- sort(
  unique(core_GLM_hits$snp_pos) # note: this uses the non rCRS positions
 ) 

core_GLM_hit_snps <- str_c(
  "pos_", 
  core_GLM_hit_snps
)

lhf_gt_meta_ds_subset_core_hits <- lhf_gt_meta_ds_subset %>%
  select(
    acc:precip_seasonality, 
    core_GLM_hit_snps
  )
...
# save this file
feather::write_feather(
  x = lhf_gt_meta_ds_subset_core_hits, 
...
ggplot2::ggsave(
  filename = "/Users/finleythomas/Library/CloudStorage/OneDrive-UniversityCollegeLondon/Results/lhf/figures/lhf_core_GLM_08_2024.png",
  plot = last_plot(),
  bg = "transparent", 
  width = 11, 
  height = 7
