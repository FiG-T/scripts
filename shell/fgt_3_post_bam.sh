...
#  3 - Processing short read sequences: Processing BAM files

################################################################################
# Summary:

#  sorted.bam -> remove low quality -> remove PCR duplicates -> index bam

#      This script is also adapted from scripts is the third step 
...
#      and uses Samtools (https://doi.org/10.1093/gigascience/giab008) to filter 
#      the sorted .bam file (processed in the previous script) to remove low 
#      quality reads. These are processed using Picard tools
#      (https://broadinstitute.github.io/picard/index.html) to remove PCR 
#      duplicates and index the bam files.  
...
#  -q 20 -bT ../../dmel-all-chromosome-r6.51.fa.fai  *_mapped_sorted.bam\    
